...
import org.jaxygen.typeconverter.exceptions.MissingConverterException;
...
public class TypeConverterFactory {
  ...
  static {
    TypeConverterFactory defaultFactory = new TypeConverterFactory();
    BasicConverters basicConvertersRegistry = new BasicConverters();
    for (TypeConverter basicConverter : basicConvertersRegistry.getConverters()) {
      defaultFactory.registerConverter(basicConverter);
    }
    factories.put(DEFAULT_FACTORY, defaultFactory);
  }
  ...
  /**
   * Get the converter which could translate an object from class from to class
   * to.
   *
   * @param <FROM> Type from which the conversion is done.
   * @param <TO> Type to which the conversion is done.
  ...
  public <FROM, TO> TO convert(final FROM from, final Class<TO> toClass) throws ConversionError {
    ...
    if (converter == null) {
      throw new MissingConverterException("Could not find converter from class " + from.getClass() + " to class " + toClass);
    }
  ...
  public <FROM, TO> TO convert(final FROM from, final Class<FROM> fromClass, final Class<TO> toClass) throws ConversionError {
    ...
    if (converter == null) {
      throw new MissingConverterException("Could not find converter from class " + fromClass + " to class " + toClass);
    }
    return (TO) converter.convert(from);
  }
}
